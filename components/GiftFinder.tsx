...
const cultures: Culture[] = ['Indian Tradition', 'Family Ritual', 'Festival Custom', 'Global Etiquette', 'Spiritual Moment', 'Modern Celebration'];
...
export function GiftFinder() {
  ...
  return (
    <section className="space-y-8">
      <div>
        <SectionLabel label="Heartfelt Idea Finder" />
        <h1 className="mt-4 font-serif text-4xl text-gold-100">Find an idea that feels truly personal.</h1>
        <p className="mt-3 max-w-2xl text-stone-text/70">
          Share the relationship, moment, feeling, and tradition around your gesture. CadeauAura will guide you toward meaningful ideas that feel personal, respectful, and heartfelt.
        </p>
      </div>
...
function FilterGroup<T extends string>({ title, options, current, onSelect }: FilterGroupProps<T>) {
  return (
    <div>
      <h2 className="mb-2 text-sm font-semibold uppercase tracking-[0.15em] text-gold-200">{title}</h2>
